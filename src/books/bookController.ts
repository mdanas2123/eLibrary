...
const uploadToCloudinary = (buffer: Buffer, options: object): Promise<UploadApiResponse> => {
...
const updateBook = async (
    ...
    try {
        ...
        if (coverImage) {
            ...
            const oldCoverImagePublicId = coverFileSplit.at(-2) + "/" + (coverFileSplit.at(-1)?.split('.').at(-2));
        ...
        if (coverImage) {
            const coverUploadResult = await uploadToCloudinary(coverImage.buffer, {
                folder: "book-covers",
                format: coverImage.mimetype.split('/').at(-1),
            });
        ...
        if (bookFile) {
            const bookUploadResult = await uploadToCloudinary(bookFile.buffer, {
                resource_type: 'raw',
                format: bookFile.mimetype.split('/').at(-1),
            });
        ...
        const updatedBook = await bookModel.findByIdAndUpdate(
            bookId,
            { title, genre, description, coverImage: updatedCoverImage, authorName, file: updatedBookFile },
            { new: true }
        );
